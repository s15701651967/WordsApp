...
public class MainActivity extends AppCompatActivity implements WordItemFragment.OnFragmentInteractionListener, WordDetailFragment.OnFragmentInteractionListener {
    ...
    private void HelpDialog(){
        new AlertDialog.Builder(this).setTitle("帮助").setMessage("当竖屏状态下用户点击单词时，会跳转到该单词的详细信息的界面，当用户长按该单词时，会出现修改和删除的功能，当用户使用查询的功能时，输入单个字母可以实现模糊查询，即列出所有包含该字母的单词。").setPositiveButton("确定", new DialogInterface.OnClickListener() {
            @Override
